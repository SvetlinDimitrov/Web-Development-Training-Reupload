...
//    System.out.println(predictPartyVictory("RDD"));
  ...
  public static String predictPartyVictory(String senate) {
    ...
    Queue<Character> active = new LinkedList<>();
    Queue<Character> voted = new LinkedList<>();

    for (char letter : senate.toCharArray()){
        if(!active.isEmpty()) {
          Character poll = active.poll();
          if(letter == poll) {
            active.add(poll);
            active.add(letter);
          }else {
        ...
        }else if (voted.isEmpty()) active.add(letter);
        else {
          Character poll = voted.poll();
          if(letter == poll) {
            voted.add(poll);
            active.add(letter);
          }else{
    ...
    if(!active.isEmpty()) return active.poll() == 'R' ? "Radiant" : "Dire";
    return voted.poll() == 'R' ? "Radiant" : "Dire";
  }
}
