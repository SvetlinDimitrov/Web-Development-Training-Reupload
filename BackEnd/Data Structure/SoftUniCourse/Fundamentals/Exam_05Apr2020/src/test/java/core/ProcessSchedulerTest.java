...
public class ProcessSchedulerTest {
    ...
    public void testInsertBefore_ShouldThrowException(){
        scheduler.insertBefore( -1 ,new ScheduledTask(21 , "gei"));
    }

    @Test
    public void testInsertBefore_ShouldBeOrderedCorrectly(){
        ScheduledTask news = new ScheduledTask(21 , "gei");
        scheduler.insertBefore( 2 ,news);
        Task task = scheduler.process();
        Assert.assertEquals(news , scheduler.process());
    }
    @Test
    public void testInsertAfter_ShouldBeOrderedCorrectly(){
        ScheduledTask news = new ScheduledTask(21 , "gei");
        scheduler.insertAfter( 1 ,news);
        System.out.println();
    }
}